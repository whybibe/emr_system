<template>
  ...
  >
    ...
    >
      ...
      <el-form-item v-if="isDisabled" label="门诊名" prop="patientName">
        <el-input v-model="depInfo.patientName" placeholder="门诊名"/>
      </el-form-item>
      <el-form-item v-if="isDisabled" label="门诊编号" prop="patientCode">
        <el-input v-model="depInfo.patientCode" placeholder="门诊编号（唯一）"/>
      </el-form-item>
      <el-form-item v-if="isDisabled" label="门诊介绍" prop="intro">
        <el-input
          type="textarea"
          :rows="4"
...
export default {
  ...
  methods: {
    setDep(dep, data) {
      this.depInfo.oldDepName = dep.depName
      this.depInfo.depName = dep.depName
      this.depInfo.oldDepCode = dep.depCode
      this.depInfo.depCode = dep.depCode
      if (data != null) {
        this.depInfo.oldPatientName = data.patientName
        this.depInfo.patientName = data.patientName
        this.depInfo.oldPatientCode = data.patientCode
        this.depInfo.patientCode = data.patientCode
        this.depInfo.oldIntro = data.intro
        this.depInfo.intro = data.intro
    ...
    submitData() {
      this.$refs.form.validate(valid => {
        if (valid) {
          departmentApi.updateDep(this.depInfo).then(res => {
            if (res.code = 200) {
